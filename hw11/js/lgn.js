...
// Generatore Normale Standard N(0,1) con Box-Muller
function boxMuller() {
  let u1 = 0, u2 = 0;
...
// Brownian Motion con Box–Muller
function generateRandomWalk() {
  const points = [];
  let y = 0;                 // X(0) = 0
  const dt = 1 / STEPS;
  const sqrtDt = Math.sqrt(dt);
  const sigma = PROB;        // λ interpretato come σ
...
const chart = new Chart(ctx, {
  ...
  options: {
    ...
    scales: {
      ...
      y: { 
        title: { display: true, text: "X(t) – Brownian Motion" },
        beginAtZero: true,
        suggestedMax: STEPS
     },
    },
